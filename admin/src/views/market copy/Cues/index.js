...
const MarketcontractABI = require('../../../ABIs/Marketplace.json')
...
const Cue = (props) => {
  ...
  async function sellNft(id, tokenID, price) {
    try {
      const response = await axios.post(
        API_URL + "/sendtransaction/v1/SellNFT",
        {
  ...
  async function buyNft(id, price) {
    try {
      const response = await axios.post(
        API_URL + "/sendtransaction/v1/BuyNFT",
        {
          Account: process.env.REACT_APP_OWNER_WALLET,
          price: price,
          id: id,
  ...
  async function cancelNft(id) {
    try {
      const response = await axios.post(
        API_URL + "/sendtransaction/v1/CancellNFT",
        {
          Account: process.env.REACT_APP_OWNER_WALLET,
          id: id,
          Type: "Cue",
