...
const Dashboard = () => {
  ...
  useEffect(async () => {
    ...
    //
    var mint = (await axios.get(baseURL + '/getmintgraph?type=month&nftType=CUE')).data
    var mintcard = (await axios.get(baseURL + '/getmintgraph?type=month&nftType=CARD')).data
    var earnMarket = (await axios.get(baseURL + '/getmarketearngraph?type=month&nftType=CUE')).data
    var earnMarketCard = (await axios.get(baseURL + '/getmarketearngraph?type=month&nftType=CARD')).data
    var earnBet = (await axios.get(baseURL + '/getearngraph?type=month&earnType=BET&amountType=SPORT')).data
    var earnEsgFee = (await axios.get(baseURL + '/getearngraph?type=month&earnType=ESG&amountType=ESGFEE')).data
    var earnEsgMaticFee = (await axios.get(baseURL + '/getearngraph?type=month&earnType=ESG&amountType=MATICFEE')).data
    var earnEsgUsdFee = (await axios.get(baseURL + '/getearngraph?type=month&earnType=ESG&amountType=USDTFEE')).data
    var totalEarnUSD = (await axios.get(baseURL + '/getTotalEarn')).data
    var totalCueCount = (await axios.get(baseURL + '/getTotalMint?type=month&nftType=CUE')).data
    var totalCardCount = (await axios.get(baseURL + '/getTotalMint?type=month&nftType=CARD')).data
    var earnSelBet = (await axios.get(baseURL + '/getSelectEarn?type=month&earnType=BET&amountType=SPORT')).data
    var earnSelEsgFee = (await axios.get(baseURL + '/getSelectEarn?type=month&earnType=ESG&amountType=ESGFEE')).data
    var earnSelEsgMaticFee = (await axios.get(baseURL + '/getSelectEarn?type=month&earnType=ESG&amountType=MATICFEE')).data
    var earnSelEsgUsdFee = (await axios.get(baseURL + '/getSelectEarn?type=month&earnType=ESG&amountType=USDTFEE')).data
    var mintSport = (await axios.get(baseURL + '/getSportMintgraph?type=month&amountType=SPORT')).data
    var mintSelAmounts = (await axios.get(baseURL + '/getTotalMintSport?type=month&amountType=SPORT')).data
    var totalCounts = (await axios.get(baseURL + '/getTotalUsers?type=month')).data
    var Users = (await axios.get(baseURL + '/getUsergraph?type=month')).data
    //
    var marketEarnCueFee = (await axios.get(baseURL + '/getSelectMarketEarn?type=month&earnType=CUE&amountType=FEE')).data
    var marketEarnCue = (await axios.get(baseURL + '/getSelectMarketEarn?type=month&earnType=CUE&amountType=EARN')).data
    var marketEarnCardFee = (await axios.get(baseURL + '/getSelectMarketEarn?type=month&earnType=CARD&amountType=FEE')).data
    var marketEarnCard = (await axios.get(baseURL + '/getSelectMarketEarn?type=month&earnType=CARD&amountType=EARN')).data
    //        
    setUserData(Users)     
  ...
  async function onNFTMintDate(type, index, nftType) {
    ...
    var mint = (await axios.get(baseURL + '/getmintgraph?type=' + type+'&nftType='+nftType)).data

    tmp[index] = true
    var totalCount = (await axios.get(baseURL + '/getTotalMint?type=' + type+'&nftType='+nftType)).data
  ...
  async function onMintEarnDate(type, index, nftType,amountType) {
    var tmp = [false, false, false, false]
    var earn = (await axios.get(baseURL + '/getmarketearngraph?type=' + type+'&nftType='+nftType)).data
    var marketEarn= (await axios.get(baseURL + '/getSelectMarketEarn?type=' + type+'&earnType='+nftType+'&amountType='+amountType)).data
  ...
  async function onEarnDate(type, index, earnType, amountType) {
    ...
    var tmp = [false, false, false, false]
    tmp[index] = true
    var earn = (await axios.get(baseURL + '/getearngraph?type=' + type+'&earnType='+earnType+'&amountType='+amountType)).data
    var earnSelAmounts = (await axios.get(baseURL + '/getSelectEarn?type=' + type+'&earnType='+earnType+'&amountType='+amountType)).data
  ...
  async function onUserCounts(type, index, earnType) {
    var totalCounts = (await axios.get(baseURL + '/getTotalUsers?type=' + type)).data
    
    
    var tmp = [false, false, false, false]
    tmp[index] = true
    var Users = (await axios.get(baseURL + '/getUsergraph?type=' + type)).data
  ...
  async function onMintDate(type, index, amountType) {
    var tmp = [false, false, false, false]
    tmp[index] = true
    var mint = (await axios.get(baseURL + '/getSportMintgraph?type=' + type+'&amountType='+amountType)).data
    var mintSelAmounts = (await axios.get(baseURL + '/getTotalMintSport?type=' + type+'&amountType='+amountType)).data
      ...
      <CCard className="mb-4">
        <CCardBody>
          <CRow>
            <CCol sm={6}>
              <h4 id="traffic" className="card-title mb-0">
                ESG - TotalSupply : {totalEsg} ESG
              </h4>
              <div className="small text-medium-emphasis">{earnDataEsg.from} ~ {earnDataEsg.to}</div>
            </CCol>
            <CCol sm={6} style={{textAlign: 'right'}}>
              <CButtonGroup role="group" aria-label="">
                <CButton color={"primary " + (earnEsgActive[0] == true ? 'active' : '')} onClick={() => onEarnDate('week', 0, 'ESG', 'ESGFEE')}>Weekly</CButton>
                <CButton color={"primary " + (earnEsgActive[1] == true ? 'active' : '')} onClick={() => onEarnDate('month', 1, 'ESG', 'ESGFEE')}>Monthly</CButton>
                <CButton color={"primary " + (earnEsgActive[2] == true ? 'active' : '')} onClick={() => onEarnDate('3month', 2, 'ESG', 'ESGFEE')}>3 Monthly</CButton>
                <CButton color={"primary " + (earnEsgActive[3] == true ? 'active' : '')} onClick={() => onEarnDate('year', 3, 'ESG', 'ESGFEE')}>Annual</CButton>
              </CButtonGroup>
            </CCol>
          </CRow>
          <CChart
            ...
            height={100}
            data={{
              labels: earnDataEsg.labels,
              datasets: [
                {
                  label: 'Earn(ESG that earned as ESG token transfer fee.)',
                  backgroundColor: 'rgb(255, 187, 68)',
                  data: earnDataEsg.earn,
                },
              ],
            }}
            options={{
                ...
                tooltip: {
                  callbacks: {
                    label: function(context) {
                      ...
                      if (context.parsed.y !== null) {
                        label += context.parsed.y.toFixed(10) + 'ESG';
                      }
                      return label;
                    }
                  }
        ...
        <CCardFooter>
          <CRow xs={{ cols: 1 }} md={{ cols: 11 }} className="text-center">
            {/* {progressExample.map((item, index) => ( */}
              <CCol className="mb-sm-2 mb-0">
                <div className="text-medium-emphasis">Total Earn : &nbsp;
                  <strong>
                    {totalEsgEarn}
                  </strong>
                  &nbsp; ESG (
