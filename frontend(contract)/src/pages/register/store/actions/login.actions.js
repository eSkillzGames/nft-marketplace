...
export const register = (nave, email, password, userName, value, file, storageRef,firebase) => async (dispatch) => {
  ...
  try {
    ...
    const response1 = await axios.post(
      "https://eskillzserver.net/sendtransaction/v1/updateCurrentSkill",
      { UserID: res.data.userResponse.uid ,Account : res.data.address}
    );
    console.log("updateCurrentSkill->", response1);
    dispatch(
      fuseActions.showMessage({
        message: "Thank you for signing up, please download the game from the links below.",
        variant: "success",
        timer:5000
      })
    );
  ...
  signInWithEmailAndPassword(auth, email, password)
    .then(async (userCredential) => {
      ...
      if(response.data == "Wallet Address does not exist now."){
        ...
        dispatch({
          type: LOGIN,
          payload: { address: response_create.data, uid: user.uid , mail: email},
        });
        nave("/home");
        // dispatch(
        //   fuseActions.showMessage({
